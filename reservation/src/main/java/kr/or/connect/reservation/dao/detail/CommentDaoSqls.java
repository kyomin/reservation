...
public class CommentDaoSqls {
	...
	public static final String SELECT_ALL_COMMENTS_BY_PRODUCT_ID_WITHOUT_COMMENT_IMAGE = "SELECT comment, reservation_user_comment_by_product_id.id AS commentId, reservation_user_comment_by_product_id.create_date AS createDate, reservation_user_comment_by_product_id.modify_date AS modifyDate, reservation_user_comment_by_product_id.product_id AS productId, reservation_date AS reservationDate, reservation_email AS reservationEmail, reservation_info.id AS reservationInfoId, reservation_name AS reservationName, reservation_tel AS reservationTelephone, score FROM reservation_info INNER JOIN " + SELECT_COMMENT_BY_PRODUCT_ID + " AS reservation_user_comment_by_product_id ON reservation_info.id = reservation_user_comment_by_product_id.reservation_info_id WHERE reservation_info.product_id = reservation_user_comment_by_product_id.product_id";
